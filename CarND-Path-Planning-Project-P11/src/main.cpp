...
int main() {
    ...
    //calculate time step for how long function has been running
    int timestep = 0;
    ...
    h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy,&lane,&ref_vel,&out_log,&timestep](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                                                                                                                                           uWS::OpCode opCode) {
        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    //car_s represents longitudinal displacement (mile marker) and car_d represents
                    //lateral displacement (which lane the car is in
                    
                    out_log << timestep << "," << car_x << "," << car_y << "," << car_s << "," << car_d << "," << car_yaw << "," << car_speed << std::endl;
